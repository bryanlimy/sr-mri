...
class Summary():
  ...
  def plot_stitched(self,
    ...
    figure, axes = plt.subplots(nrows=shape[0],
                                ncols=3,
                                figsize=(8, 3) if shape[0] == 1 else
                                (7.5, 2.5 * shape[0]),
                                squeeze=False,
                                dpi=self.dpi)
  ...
  def plot_attention_masks(self,
    ...
    figure, axes = plt.subplots(nrows=num_images,
                                ncols=1,
                                gridspec_kw={
                                    'hspace': 0.125,
                                    'wspace': 0.1
    ...
    for i in range(len(gate_inputs)):
      gate_input = utils.to_numpy(torch.mean(gate_inputs[f'gate_{i}'][0],
                                             dim=0))
      ...
      set_xticks(axis=axes[row],
                 ticks_loc=np.linspace(0, superimpose.shape[1] - 1, 5))
      set_yticks(axis=axes[row],
    ...
    set_xticks(axis=axes[row],
               ticks_loc=np.linspace(0, outputs.shape[1] - 1, 5))
    set_yticks(axis=axes[row],
