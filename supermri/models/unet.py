...
class AttentionGate(nn.Module):
  ...
  def __init__(self, in_channels: int, normalization: str = 'instancenorm'):
    ...
    self.gating_conv = nn.Conv2d(in_channels=in_channels,
                                 out_channels=in_channels,
                                 kernel_size=1)
    self.gating_norm = model_helper.normalization(normalization)(in_channels)

    self.input_conv = nn.Conv2d(in_channels=in_channels,
                                out_channels=in_channels,
                                kernel_size=1)
    self.input_norm = model_helper.normalization(normalization)(in_channels)

    self.relu = model_helper.activation('relu')()
    self.conv = nn.Conv2d(in_channels=in_channels,
                          out_channels=1,
                          kernel_size=1)
    self.norm = model_helper.normalization(normalization)(1)
...
class UpScale(nn.Module):
  ...
  def __init__(self,
    ...
    if use_attention_gate:
      self.attention_gate = AttentionGate(in_channels=in_channels // 2,
                                          normalization=normalization)
  ...
  def __init__(self,
    ...
    self.down_blocks = nn.ModuleList([
        DownScale(in_channels=self.filters[i],
                  ...
                  normalization=normalization,
                  activation=activation,
                  dropout=dropout) for i in range(len(self.filters) - 1)
    ])
