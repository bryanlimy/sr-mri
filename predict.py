...
def load_args(args):
  ''' Loads settings from output_dir/args.json '''
  utils.load_args(args, filename=os.path.join(args.output_dir, 'args.json'))
  args.input_shape = tuple(args.input_shape)
  args.scan_shape = tuple(args.scan_shape)

  # create directory for super-resolution outputs
  if args.sr_dir is None:
    args.sr_dir = os.path.join(args.input_dir, 'sr')
  if os.path.exists(args.sr_dir) and not args.overwrite:
    raise FileExistsError(f'--sr_dir {args.sr_dir} already exits.')
...
def main(args):
  ...
  if not os.path.exists(args.output_dir):
    raise FileNotFoundError(f'--output_dir {args.output_dir} not found.')
  ...
  for filename in filenames:
    ...
    for batch in tqdm(data_loader, desc=subject.name):
      inputs, _ = data.prepare_batch(batch,
                                     dim=args.slice_dim,
                                     device=args.device)
    ...
    save_mat(args,
             filename=os.path.join(args.sr_dir, f'{subject.name}.mat'),
             scan=output_tensor)

  print(f'\n{len(filenames)} super-resolution scans stored at {args.sr_dir}')
...
if __name__ == '__main__':
  ...
  parser.add_argument('--overwrite',
                      action='store_true',
                      help='overwrite existing data in --sr_dir')
